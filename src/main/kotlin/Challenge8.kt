...
sealed class AcceptedCurrency {
    ...
    // Challenge
    companion object {
        fun checkSufficientFunds(currentFunds: List<AcceptedCurrency>, totalCost: Float): Boolean =
            currentFunds.fold(0.0) { sum, amount -> sum + amount.valueInDollars } >= totalCost
    }
}
...
fun main() {
    ...
    val currentBalance = AcceptedCurrency.checkSufficientFunds(listOf(currency, dollars), 1500f)
    println("You ${if (currentBalance) "do" else "do not"} have enough money to buy that!")
}